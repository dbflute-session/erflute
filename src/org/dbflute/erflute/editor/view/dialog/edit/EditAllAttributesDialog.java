package org.dbflute.erflute.editor.view.dialog.edit;
...
import org.dbflute.erflute.core.widgets.CenteredContentCellPaint;
...
public class EditAllAttributesDialog extends AbstractDialog implements EditableTable {
    ...
    private void createTable(Composite composite) {
        ...
        this.attributeTable = new Table(composite, SWT.SINGLE | SWT.BORDER | SWT.FULL_SELECTION);
        ...
        TableColumn columnForeignKey = new TableColumn(this.attributeTable, SWT.CENTER);
    ...
    private void column2TableItem(ERTable table, ERColumn column, TableItem tableItem) {
        ...
        if (column instanceof NormalColumn) {
            ...
            if (normalColumn.isPrimaryKey()) {
                tableItem.setImage(colCount, Activator.getImage(ImageKey.PRIMARY_KEY));
            } else {
            ...
            colCount++;
            if (normalColumn.isForeignKey()) {
                tableItem.setImage(colCount, Activator.getImage(ImageKey.FOREIGN_KEY));
            ...
            if (normalColumn.isNotNull()) {
                if (normalColumn.isPrimaryKey()) {
                    tableItem.setImage(colCount, Activator.getImage(ImageKey.CHECK_GREY));
                } else {
                    tableItem.setImage(colCount, Activator.getImage(ImageKey.CHECK));
                }
        ...
        if (column instanceof NormalColumn) {
            ...
            if (xy.x == 4) {
                ...
                if (xy.x == 0) {
                    ...
                    SqlType selectedType = SqlType.valueOf(database, ((Combo) control).getText());
