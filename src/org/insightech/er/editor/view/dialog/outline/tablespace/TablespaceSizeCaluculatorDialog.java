...
public class TablespaceSizeCaluculatorDialog extends AbstractDialog implements EditableTable {
    ...
    private void calculate() {
        double bytesOfBlockHeader =
                this.getValue(this.kcbhText) + this.getValue(this.ub4Text) + this.getValue(this.ktbbhText)
                        + ((initrans - 1) * this.getValue(this.ktbitText)) + this.getValue(this.kdbhText);
        ...
        double bytesOfDataPerBlock =
                Math.ceil((this.getValue(this.dbBlockSizeText) - bytesOfBlockHeader) * (1 - (this.pctfree / 100)))
        ...
        for (ERTable table : this.tableList) {
            ...
            int totalBytes = (int) (totalBlockNum * this.getValue(this.dbBlockSizeText));
