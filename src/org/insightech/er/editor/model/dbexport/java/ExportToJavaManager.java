...
public class ExportToJavaManager {
    ...
    static {
        try {
            ...
            HIBERNATE_TEMPLATE = loadResource("hibernate" + File.separator + "hbm");
            HIBERNATE_PROPERTY = loadResource("hibernate" + File.separator + "@property");
            HIBERNATE_ID = loadResource("hibernate" + File.separator + "@id");
            HIBERNATE_COMPOSITE_ID = loadResource("hibernate" + File.separator + "@composite_id");
            HIBERNATE_COMPOSITE_ID_KEY = loadResource("hibernate" + File.separator + "@composite_id_key");
    ...
    public ExportToJavaManager(ExportJavaSetting exportJavaSetting, ERDiagram diagram) {
    ...
    public void doProcess() throws IOException, InterruptedException {
        for (ERTable table : diagram.getDiagramContents().getContents().getTableSet().getList()) {
            ...
            if (this.exportJavaSetting.isWithHibernate()) {
                ...
                this.writeOut(File.separator + this.packageDir + File.separator + className + ".hbm.xml", hbmContent);
            }

            String content = this.generateContent(diagram, table, compositeIdClassName);

            this.writeOut(File.separator + this.packageDir + File.separator + className + ".java", content);

        }
    }

    protected String getClassName(ERTable table) {
        return this.getCamelCaseName(table) + this.getCamelCaseName(this.exportJavaSetting.getClassNameSuffix(), true);
    }
    ...
    private static String loadResource(String templateName) throws IOException {
        ...
        InputStream in = ExportToJavaManager.class.getClassLoader().getResourceAsStream(resourceName);
        ...
        try {
            ...
            for (String keyword : KEYWORDS) {
                content = content.replaceAll(keyword, ResourceString.getResourceString(keyword));
            }
    ...
    private String generateContent(ERDiagram diagram, ERTable table, String compositeIdClassName) throws IOException {
    ...
    private String replacePropertiesInfo(String content, ERTable table, List<NormalColumn> columns, List<NodeElement> referringElementList,
        ...
        if (compositeIdClassName != null) {
            this.addCompositeIdContent(properties, PROPERTIES, compositeIdClassName, table);
            this.addCompositeIdContent(setterGetters, SETTER_GETTER, compositeIdClassName, table);
        }
    ...
    private String replaceHashCodeEqualsInfo(String content, ERTable table, String compositeIdClassName) throws IOException {
        if (compositeIdClassName != null) {
            ...
            this.addCompositeIdContent(hashCodes, HASHCODE_LOGIC, compositeIdClassName, table);
            this.addCompositeIdContent(equals, EQUALS_LOGIC, compositeIdClassName, table);
    ...
    private String replaceClassInfo(String content, String classDescription, String className, String classNameSufix) {
        if (Check.isEmptyTrim(this.exportJavaSetting.getPackageName())) {
            content = content.replaceAll("package @package;\r\n\r\n", "");

        } else {
            content = content.replaceAll("@package", this.exportJavaSetting.getPackageName());
        }
    ...
    private String replaceExtendInfo(String content) throws IOException {
        if (Check.isEmpty(this.exportJavaSetting.getExtendsClass())) {
            ...
            if (index == -1) {
                extendsClassWithoutPackage = this.exportJavaSetting.getExtendsClass();

            } else {
                extendsClassWithoutPackage = this.exportJavaSetting.getExtendsClass().substring(index + 1);
            }

            content = content.replaceAll("@extendsClassWithoutPackage", extendsClassWithoutPackage);
            content = content.replaceAll("@extendsClass", this.exportJavaSetting.getExtendsClass());
        }
    ...
    private String replaceConstructorInfo(String content) {
        ...
        for (String tableName : this.sets) {
            ...
            constructor.append(this.getCamelCaseName(tableName, true)
                    + this.getCamelCaseName(this.exportJavaSetting.getClassNameSuffix(), true));
            constructor.append(">();\r\n");
        }
    ...
    private void addContent(StringBuilder contents, String template, NormalColumn normalColumn) {
        ...
        if (normalColumn.isForeignKey()) {
            NormalColumn referencedColumn = normalColumn.getRootReferencedColumn();

            ERTable referencedTable = (ERTable) referencedColumn.getColumnHolder();
    ...
    private void addContent(StringBuilder contents, String template, TableView tableView) {
        ...
        value =
                value.replaceAll(
                        "@setType",
                        "Set<" + this.getCamelCaseName(tableView.getPhysicalName(), true)
                                + this.getCamelCaseName(this.exportJavaSetting.getClassNameSuffix(), true) + ">");
        value =
                value.replaceAll(
                        "@type",
        ...
        value = value.replaceAll("@physicalColumnName", this.getCamelCaseName(tableView.getPhysicalName(), false));
        value = value.replaceAll("@PhysicalColumnName", this.getCamelCaseName(tableView.getPhysicalName(), true));
    ...
    private void writeOut(String dstPath, String content) throws IOException {
        dstPath = this.exportJavaSetting.getJavaOutput() + File.separator + "src" + dstPath;
    ...
    private String generateHbmContent(ERDiagram diagram, ERTable table, String compositeIdClassName) throws IOException {
        String content = HIBERNATE_TEMPLATE;

        content = content.replaceAll("@package", this.exportJavaSetting.getPackageName());
        content = content.replaceAll("@PhysicalTableName", this.getCamelCaseName(table));
        content = content.replaceAll("@suffix", Format.null2blank(this.exportJavaSetting.getClassNameSuffix()));
        ...
        if (table.getPrimaryKeySize() == 1) {
            ...
            for (NormalColumn column : table.getPrimaryKeys()) {
                ...
                key = key.replaceAll("@type", this.getFullClassName(column.getType()));
