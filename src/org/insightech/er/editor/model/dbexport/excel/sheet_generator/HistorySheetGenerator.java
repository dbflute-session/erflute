...
public class HistorySheetGenerator extends AbstractSheetGenerator {
    ...
    public void generate(IProgressMonitor monitor, HSSFWorkbook workbook, int sheetNo, boolean useLogicalNameAsSheetName,
        ...
        HSSFSheet newSheet = createNewSheet(workbook, sheetNo, sheetName, sheetNameMap);

        sheetObjectMap.put(workbook.getSheetName(workbook.getSheetIndex(newSheet)), new StringObjectModel(sheetName));
    ...
    public void setHistoryListData(HSSFWorkbook workbook, HSSFSheet sheet, Map<String, ObjectModel> sheetObjectMap, ERDiagram diagram) {
        ...
        if (cellLocation != null) {
            ...
            ColumnTemplate columnTemplate = this.loadColumnTemplate(workbook, sheet, cellLocation);
            int order = 1;
                ...
                for (int columnNum : columnTemplate.columnTemplateMap.keySet()) {
                    ...
                    if (KEYWORD_ORDER.equals(template)) {
                        ...
                        if (KEYWORD_DATE.equals(template)) {
                            DateFormat format = new SimpleDateFormat(this.keywordsValueMap.get(KEYWORD_DATE_FORMAT));
                            try {
