...
public class IndexSheetGenerator extends AbstractSheetGenerator {
    ...
    public void generate(IProgressMonitor monitor, HSSFWorkbook workbook, int sheetNo, boolean useLogicalNameAsSheetName,
        ...
        for (ERTable table : diagram.getDiagramContents().getContents().getTableSet()) {
            ...
            for (Index index : table.getIndexes()) {
                ...
                sheetObjectMap.put(workbook.getSheetName(workbook.getSheetIndex(newSheet)), index);
    ...
    public void setIndexData(HSSFWorkbook workbook, HSSFSheet sheet, Index index) {
        ...
        POIUtils.replace(sheet, KEYWORD_LOGICAL_TABLE_NAME,
                this.getValue(this.keywordsValueMap, KEYWORD_LOGICAL_TABLE_NAME, index.getTable().getLogicalName()));
        POIUtils.replace(sheet, KEYWORD_INDEX_DESCRIPTION,
                this.getValue(this.keywordsValueMap, KEYWORD_INDEX_DESCRIPTION, index.getDescription()));
        ...
        if (cellLocation != null) {
            ...
            if (this.columnTemplate == null) {
                this.columnTemplate = loadColumnTemplate(workbook, sheet, cellLocation);
            }
            ...
            for (NormalColumn normalColumn : index.getColumns()) {
                ...
                this.setColumnData(this.keywordsValueMap, columnTemplate, row, normalColumn, index.getTable(), order);
                order++;
            }
